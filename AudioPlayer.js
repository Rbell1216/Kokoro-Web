...
export class AudioDiskSaver {
  ...
  async initSave() {
    try {
      ...
      // --- THIS IS THE FIX ---
      // Get the slider's value and calculate the real speed
      const sliderValue = parseFloat(document.getElementById('speed-slider').value);
      const realSpeed = getRealSpeed(sliderValue);
      
      // Write the header using the calculated speed
      await this.writeWavHeader(realSpeed);
      // --- END OF FIX ---
  ...
  // This function now accepts the calculated real speed
  async writeWavHeader(speed = 1.0) {
    const headerBuffer = new ArrayBuffer(44);
    const view = new DataView(headerBuffer);

    // --- THIS IS THE FIX ---
    const effective_sample_rate = Math.round(SAMPLE_RATE * speed);
